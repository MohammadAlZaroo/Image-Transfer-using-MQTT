    ...
    for (int x = kOffset; x < width - kOffset; x++) {
      ...
      for (int ky = 0; ky < kSize; ky++) {
        for (int kx = 0; kx < kSize; kx++) {
          int pixel = image.getPixel(x + kx - kOffset, y + ky - kOffset) & 0xFF;
...
static int otsuThreshold(List<int> histogramCounts) {
  int total = histogramCounts.reduce((a, b) => a + b); // Total number of pixels
  int top = 256;
  int sumB = 0;
  int wB = 0;
  double maximum = 0.0;
  ...
  int sum1 =
      List.generate(top, (i) => i * histogramCounts[i]).reduce((a, b) => a + b);
